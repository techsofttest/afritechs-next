...
export default function QualitySecurity({ data }: QualitySecurityProps) {
  const qualityTitle = data?.quality_title || "";
  const qualityDesc = data?.quality_desc || "";
  const hseTitle = data?.hse_title || "";
  const hseDesc = data?.hse_desc || "";

  if (!qualityTitle && !qualityDesc && !hseTitle && !hseDesc) return null;
        ...
        {(qualityTitle || qualityDesc) && (
          <div className="p-6 md:p-10 border border-gray-200 rounded-2xl bg-white flex flex-col justify-between">
            <div>
              ...
              {qualityTitle && <h3 className="text-lg md:text-xl font-semibold mb-3 text-[#0c2847]">{qualityTitle}</h3>}
              {qualityDesc && (
        ...
        {(hseTitle || hseDesc) && (
          <div className="p-6 md:p-10 border border-gray-200 rounded-2xl bg-white flex flex-col justify-between">
            <div>
              ...
              {hseTitle && <h3 className="text-lg md:text-xl font-semibold mb-3 text-[#0c2847]">{hseTitle}</h3>}
              {hseDesc && (
                <p className="text-gray-700 text-sm md:text-base leading-relaxed font-medium mb-4">
                  {hseDesc}
                </p>
