...
export default function MissionVision({ data }: MissionVisionProps) {
  const sectionTitle = data?.section_title || "Mission & Vision";
  const missionTag = data?.mission_tag || "Notre Mission";
  const missionTitle = data?.mission_title || "Quality & Cost Effective Service";
  const missionDesc = data?.mission_desc || "Our mission is to provide Quality & Cost Effective End-to-End Service, thereby achieving our target of Customer Satisfaction to a greater extent.";
  const visionTag = data?.vision_tag || "Notre Vision";
  const visionTitle = data?.vision_title || "Globally Recognized Brand";
  const visionDesc = data?.vision_desc || "To achieve year-on-year growth by being a globally recognized brand and the partner of choice for providing complete Agricultural and Automotive solutions.";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-[#0c2847] text-white p-6 md:p-12 rounded-2xl border border-[#0c2847] flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-xl md:text-2xl font-semibold mb-3 md:mb-4">{missionTitle}</h3>
        ...
        <div className="bg-gray-50 border border-gray-200 p-6 md:p-12 rounded-2xl flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-xl md:text-2xl font-semibold mb-3 md:mb-4 text-[#0c2847]">{visionTitle}</h3>
